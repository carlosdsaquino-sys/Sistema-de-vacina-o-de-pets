...
export function DashboardPage() {
  ...
  async function loadDashboard() {
    ...
    for (const app of appStats || []) {
      vaccineCountMap.set(
        app.vaccine_id,
        (vaccineCountMap.get(app.vaccine_id) || 0) + 1
      );
    }
    ...
    const vaccineStats = Array.from(vaccineCountMap.entries())
      .map(([vid, count]) => ({
        nome:
          vaccines.data?.find((v) => v.id === vid)?.nome ||
          'Desconhecida',
        count,
      }))
  ...
  const maxVaccineCount = Math.max(
    ...data.vaccineStats.map((v) => v.count),
    1
  );
          ...
          <CardBody>
            <div className="space-y-2">
              {data.lowStockVaccines.map((v) => (
                <div
                  key={v.id}
                  className="flex items-center justify-between rounded-lg border border-amber-100 dark:border-amber-900/40 bg-amber-50/30 dark:bg-amber-950/10 p-3"
                >
